...
def getHeaderTones(inputFile, outputFile):
   ...
   if not firstValue == ['HeaderCounter', 'Headline', 'Body ID', 'Stance']:
      raise ValueError("The input file's first row does not comform to the documented format")
   outputWriter.writerow( ('HeaderCounter', 'Body ID', 'anger', 'disgust', 'fear', 'joy', 'sadness', 'Stance') )
...
def getBodyTones(inputFile, outputFile):
   ...
   outputWriter = csv.writer(outputFile)
   firstValue = next(inputReader)
   if not firstValue == ['BodyCounter', 'Body ID', 'articleBody']:
      raise ValueError("The input file's first row does not comform to the documented format")
   outputWriter.writerow( ('BodyCounter', 'Body ID', 'anger', 'disgust', 'fear', 'joy', 'sadness') )

   globalVars.init()
   my_headers = {"Content-Type": 'text/plain'}
   auth = requests.auth.HTTPBasicAuth(globalVars.userName, globalVars.password)
   for line in inputReader:
      r = requests.post(globalVars.url, data=line[2], headers=my_headers, auth=auth)
      while r.status_code == requests.codes.too_many:
         time.sleep(60)
         r = requests.post(globalVars.url, data=line[2], headers=my_headers, auth=auth)
      if not r.status_code == requests.codes.ok:
